<?php
include_once 'includes/connexion_db.php'; //fourni la fonction de connexion connect();
...
if (isset($_GET['id'])) {
    ...
    //var_dump($joueur);   pr vérifier si pas de bug
}
...
//si la connexion n'existe pas, on DOIT l'initialiser avant l'étape de requête (connexion mySQL)
    if (!isset($db)) $db=connect ();

    $query = $db->prepare('UPDATE joueur SET prenom = :prenom, nom = :nom, age = :age, maillot = :maillot, equipe = :equipe WHERE id = :id');
    ...
    //redirection vers la liste des joueurs, header fct native de php
